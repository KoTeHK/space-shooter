...
class Menu():
    ...
    def draw_menu(window):
        ...
        window.blit(self.RENDER1, (self.BUTTON1.x + 20, self,BUTTON1.y + 10))
        window.blit(self.RENDER2, (self.BUTTON2.x + 20, self,BUTTON2.y + 10))
...
class Level():
    ...
    def next_level(self, window):
        text = f"Ви перешли на новий рівень (self.LVL)"
...
class Hero(Sprite):
    ...
    def move(self, window):
        if key[pygame.K_w] self.MOVE["UP"] and self.x > 0:
            self.y -= self.SPEED
        elif key[pygame.K_w] self.MOVE["DOWN"] and self.y + self.height < setting_win["HEIGHT"]:
            self.y += self.SPEED
...
class Boss(Sprite):
    ...
    def move(self, window):
        ...
        if pygame.time.get_ticks() - self.START_SHOT > 1500:
            self.START_SHOT = pygame.time.get_ticks()
            bullet_boss_list.append(Bullet(bullet_image, 3, x= self.x + self.width // 2 - 50, y= self.y + self.height, width= 5, height= 10))
            bullet_boss_list.append(Bullet(bullet_image, 3, x= self.x + self.width // 2 + 40, y= self.y + self.height, width= 5, height= 10))
